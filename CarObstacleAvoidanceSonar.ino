...
#define TRIG 5
#define ECHO 18
#define OBSTACLE_THRESHOLD 60 // stop forward if <=60 cm
...
bool movingForward = false; // track forward button press
...
void backward(int s) {
  movingForward = false; // disable forward-only obstacle logic
...
void handleCommand() {
  ...
  else if (cmd == "hard_stop") hardStop();
...
<div class="container">
  ...
  <div class="section">
    ...
    <button class="stop" onclick="send('hard_stop')">HARD STOP</button>
  </div>
...
void loop() {
  ...
  // Automatic obstacle avoidance for forward only
  if (movingForward) {
    float distance = readDistance();
    if (distance > 0 && distance <= OBSTACLE_THRESHOLD) {
      Serial.print("Obstacle detected! Distance: ");
      Serial.println(distance);
      hardStop(); // stop immediately
      // optional: small backup if you want
      // backward(150);
      // delay(300);
      // stopMotor();
    }
